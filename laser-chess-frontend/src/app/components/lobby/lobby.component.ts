...
export class LobbyComponent implements OnInit, OnDestroy {
  ...
  connectWebsocket(lobbyId: string) {
    this.subject.asObservable().subscribe(
      (msg) => {
        if ((<Lobby>msg).game_id) this.refreshLobbyState(msg);
      },
      (err) => {
        console.error(err);
      }
  ...
  async startGame() {
    if (
      ...
      this.player_one &&
      this.player_two &&
      this.username == this.lobby.player_one_username
    ) {
      await this.lobbyService.startGame(
        this.lobby.game_id,
        this.player_one,
        this.player_two,
        this.lobby.is_ranked,
        this.isTimed,
        this.timesMap.get(this.selectedTime1) || -1,
        this.timesMap.get(this.selectedTime2) || -1
      );
