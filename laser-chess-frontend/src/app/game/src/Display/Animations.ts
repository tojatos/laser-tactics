...
export class Animations {
  ...
  async movePiece(
    ...
    const destination = board.getCellByCoordinates(
    ...
    const numOfIterations =
      this.getTranslationValue(origin.canvasCoordinates.x, destination.canvasCoordinates.x) == 0
    ...
    let validCellsArray = origin.auxiliaryPiece
      ? board.cells
      : this.cellsExcludingPieces(board, [origin]);
    ...
    return new Promise<void>((resolve) => {
      const interval = () => {
        ...
        if (iteration >= numOfIterations) {
          lastAction();
          resolve();
        } else window.requestAnimationFrame(interval);
      };
  ...
  async rotatePiece(
    canvas: Canvas,
    board: Board,
    atCell: Cell | undefined,
    byDegrees: number,
    isReverse: boolean,
    showAnimations: boolean,
    enableSounds: boolean,
    initialRotationDifference = 0
  ): Promise<void> {
    const piece = cloneDeep(atCell?.piece);

    if (!piece || !atCell) return;
  ...
  async laserAnimation(
    ...
    const lastAction = () => {
      for (const position of positions) {
        ...
        const toCell = board.getCellByCoordinates(
          Math.min(8, Math.max(0, position[1].x)),
          Math.min(8, Math.max(0, position[1].y))
        );

        if (fromCell && toCell) {
          this.drawings.drawLaserLine(
            canvas,
            fromCell.canvasCoordinates,
            toCell.canvasCoordinates,
            isReverse
          );
      ...
      const interval = () => {
        for (const position of positions) {
          ...
          if (fromCell && toCell) {
            ...
            if (position[1].x > 8)
              finalDest = {
                x: toCell.canvasCoordinates.x + canvas.blockSize,
                y: toCell.canvasCoordinates.y,
              };
            ...
            if (position[1].y > 8)
              finalDest = {
                x: toCell.canvasCoordinates.x,
                y: toCell.canvasCoordinates.y - canvas.blockSize,
              };
            ...
            this.drawings.drawLaserLine(
              canvas,
              fromCell.canvasCoordinates,
              this.laserAnimationStep(fromCell.canvasCoordinates, finalDest, laserIncrement),
              isReverse
            );
          }
        }
            ...
            for (const pixel of row) {
              ...
              if (standardDev > 0 && canvas.ctx) {
                const positionX = Math.round(
                  Math.abs(Chance().normal({ mean: 0, dev: standardDev }))
                );
                ...
                if (
                  positionX != 0 &&
                  positionY != 0 &&
                  this.isInCanvasBoundaries(canvas, pixel.originCoordinates)
                ) {
                  canvas.ctx.clearRect(
                    pixel.originCoordinates.x,
                    pixel.originCoordinates.y,
                    pixelSize,
                    pixelSize
                  );
  ...
  private cellsExcludingPieces(board: Board, cells: Cell[]) {
    return board.cells.filter((c) =>
      cells.every(
        (cl) => cl.piece != board.getCellByCoordinates(c.coordinates.x, c.coordinates.y)?.piece
      )
    );
  }

  private inRotationVicinity(
    currentRotation: number,
    desiredRotation: number,
    deegresPerFrame: number
  ) {
    if (currentRotation < 0) currentRotation = 360 + currentRotation;

    if (desiredRotation < 0) desiredRotation = 360 + desiredRotation;
  ...
  private changePosition(
    piece: Piece,
    originCooridantes: Coordinates,
    destinationCoordinates: Coordinates,
    redrawDistance: number,
    fun: (x: number) => number
  ) {
    const translationValueX = this.getTranslationValue(
      originCooridantes.x,
      destinationCoordinates.x
    );
    const translationValueY = this.getTranslationValue(
      originCooridantes.y,
      destinationCoordinates.y
    );
  ...
  isInCanvasBoundaries(canvas: Canvas, coor: Coordinates): boolean {
    return (
      coor.x >= 0 && coor.x <= canvas.canvas.width && coor.y >= 0 && coor.y <= canvas.canvas.height
    );
  }
