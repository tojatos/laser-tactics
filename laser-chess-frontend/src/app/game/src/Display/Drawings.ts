...
export class Drawings {
    ...
    if (canvas.ctx) {
      ...
      if (pieceImage)
        canvas.ctx.drawImage(
          pieceImage,
          this.pieceDrawingOriginCoordinates(canvas.blockSize).x,
          this.pieceDrawingOriginCoordinates(canvas.blockSize).y,
          canvas.blockSize * PIECE_SIZE_SCALE,
          canvas.blockSize * PIECE_SIZE_SCALE
        );
      canvas.ctx.restore();
    }
  ...
  highlightCell(
    ...
    if (cell && canvas.ctx) {
      ...
      const cellCoor = isReverse
        ? this.flipPosition(
            cell.canvasCoordinates,
            canvas.ctx.canvas.width,
            canvas.ctx.canvas.height
          )
        : cell.canvasCoordinates;
      ...
      canvas.ctx.rect(
        this.cellDrawingOriginCoordinates(canvas.blockSize).x,
        this.cellDrawingOriginCoordinates(canvas.blockSize).y,
        canvas.blockSize,
        canvas.blockSize
      );
  ...
  drawLaserLine(canvas: Canvas, from: Coordinates, to: Coordinates, isReverse: boolean): void {
    if (canvas.ctx) {
      from = isReverse
        ? this.flipPosition(from, canvas.ctx.canvas.width, canvas.ctx.canvas.height)
        : from;
      to = isReverse
        ? this.flipPosition(to, canvas.ctx.canvas.width, canvas.ctx.canvas.height)
        : to;
      canvas.ctx.save();
  ...
  drawLaserCorner(canvas: Canvas, at: Coordinates, isReverse: boolean): void {
    if (canvas.ctx) {
      canvas.ctx.save();
      at = isReverse
        ? this.flipPosition(at, canvas.ctx.canvas.width, canvas.ctx.canvas.height)
        : at;
      canvas.ctx.translate(at.x, at.y);
      canvas.ctx.fillStyle = 'red';
      canvas.ctx.fillRect(
        -this.laserThickness / 2,
        -this.laserThickness / 2,
        this.laserThickness,
        this.laserThickness
      );
  ...
  getPieceIndividualPixels(canvas: Canvas, cell: Cell, size: number, isReverse: boolean) {
    if (canvas.ctx) {
      ...
      const cellCoordinates = isReverse
        ? this.flipPosition(
            cell.canvasCoordinates,
            canvas.ctx.canvas.width,
            canvas.ctx.canvas.height
