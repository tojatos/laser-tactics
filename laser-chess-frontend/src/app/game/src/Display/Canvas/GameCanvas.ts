...
export class GameCanvas extends Canvas {
  ...
  private async selectableCellEvent(selectedCell: Cell | undefined, board: Board) {
    ...
    if (board.selectedCell && selectedCell && this.mediator?.currentRotation == 0) {
      ...
      this.gameService.movePiece(
        this.gameId,
        board.selectedCell.coordinates,
        selectedCell.coordinates
      );
      if (board.isMyTurn()) this.interactable = true;
  ...
  async rotationButtonPressed(
    board: Board,
    degree: number,
    initialRotationDifference: number
  ): Promise<void> {
    const selectedCell = board.selectedCell;

    if (selectedCell) {
      this.interactable = false;
      await this.animations.rotatePiece(
        this,
        board,
        selectedCell,
        degree,
  ...
  private selectCellEvent(selectedCell: Cell, board: Board): void {
    ...
    selectedCell.piece
      ...
      .forEach((c) =>
        this.drawings.showPossibleMove(this, c, this.isReversed, EventsColors.MOVE_EVENT)
      );
  }

  highlightPossibleMoves(board: Board): void {
    if (board.selectedCell)
      board.selectedCell.piece
        ?.getPossibleMoves(board, board.selectedCell)
        .forEach((c) =>
          this.drawings.showPossibleMove(this, c, this.isReversed, EventsColors.MOVE_EVENT)
        );
  }
