...
export class GameActions {
  ...
  newCellSelectedEvent(board: Board): void {
    ...
    if (selectedCell?.piece) {
      ...
      if (selectedCell.piece.piece_type == PieceType.LASER) this.laserActive = true;
    }
  }
  ...
  async rotationPressed(board: Board, degree: number): Promise<void> {
    ...
    if (this.rotation == 0) {
      if (board.selectedCell?.piece?.piece_type == PieceType.LASER) this.laserActive = true;
  ...
  async rotatePieceToInitialPosition(board: Board): Promise<void> {
    if (this.rotation != 0) {
      await this.mediator.sendRotationInfo(
        board,
        this.rotation < 180 ? -this.rotation : 360 - this.rotation,
        this.rotation
      );
      this.rotation = 0;
      this.mediator.drawGameOnGameCanvas(board);
