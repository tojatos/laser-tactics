...
export class Cell implements CellInterface {
  ...
  init(
    ...
    this.piece =
      ...
      new Piece(
        piece.piece_owner,
        piece.piece_type,
        piece.rotation_degree,
        this.cloneCoordinates(this.canvasCoordinates)
      );
    this.auxiliaryPiece =
      auxiliaryPiece &&
      new Piece(
        auxiliaryPiece.piece_owner,
        auxiliaryPiece.piece_type,
        auxiliaryPiece.rotation_degree,
        this.cloneCoordinates(this.canvasCoordinates)
      );
  }
  ...
  acceptNewPiece(piece: Piece): void {
    ...
    if (this.piece?.piece_type == PieceType.HYPER_SQUARE) this.auxiliaryPiece = piece;
  ...
  changeCanvasCoordinates(size: number): void {
    ...
    if (this.piece) this.piece.currentCoordinates = this.cloneCoordinates(this.canvasCoordinates);
  }
  ...
  private canvasCoordinatesSetter(size: number) {
    return {
      x: this.coordinates.x.valueOf() * size + size / 2,
      y: size * (ROWS - 1) - this.coordinates.y.valueOf() * size + size / 2,
    };
  }
