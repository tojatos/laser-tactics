...
export class Board implements BoardInterface {
  ...
  initBoard(gameState: GameState, blockSize: number): void {
    ...
    gameState.board.cells.forEach((c) =>
      this.cells.push(new Cell(c.coordinates, c.piece, c.auxiliaryPiece, blockSize))
    );
  }

  selectCell(cell: Cell | undefined): void {
    if (cell && this.cells.includes(cell) && cell.piece) this.selectedCell = cell;
  }
  ...
  getSelectableCellByCoordinates(
    x: number,
    y: number,
    ...
    if (!this.selectedCell)
      return this.cells.find(
        (c) =>
          c.coordinates.x == x &&
          c.coordinates.y == y &&
          c.piece?.piece_owner == this.parsePlayerIdToPlayerNumber(owner)
      );
    return this.selectedCell.piece
      ?.getPossibleMoves(this, this.selectedCell)
      .find((c) => c?.coordinates.x == x && c.coordinates.y == y);
  }
  ...
  movePiece(origin: Coordinates, destination: Coordinates): void {
    ...
    if (originCell && destinationCell && originCell.piece) {
      if (
        (originCell.piece.piece_type == PieceType.HYPER_SQUARE ||
          originCell.piece.piece_type == PieceType.HYPER_CUBE) &&
        originCell.auxiliaryPiece
      ) {
  ...
  executeEvent(gameEvent: GameEvent): void {
    switch (gameEvent.event_type) {
      case GameEvents.PIECE_ROTATED_EVENT:
        this.rotatePiece(gameEvent.rotated_piece_at, gameEvent.rotation);
        break;
      case GameEvents.PIECE_MOVED_EVENT:
        this.movePiece(gameEvent.moved_from, gameEvent.moved_to);
        break;
      case GameEvents.TELEPORT_EVENT:
        this.movePiece(gameEvent.teleported_from, gameEvent.teleported_to);
        break;
      case GameEvents.PIECE_DESTROYED_EVENT:
        this.removePiece(gameEvent.destroyed_on);
        break;
    }
  }
  ...
  get turnOfPlayer(): PlayerType {
    const turnOfPlayer =
      Math.round(this.currentTurn / 2) % 2 == 0 ? this.playerTwo : this.playerOne;
  ...
  get playerNum(): PlayerType {
    ...
    if (player == this.playerOne) return PlayerType.PLAYER_ONE;
    else if (player == this.playerTwo) return PlayerType.PLAYER_TWO;
  ...
  setGameState(gameState: GameState, blockSize: number): void {
    if (this.blockSize) this.initBoard(gameState, blockSize);
  }

  removePiece(at: Coordinates): void {
    const cell = this.getCellByCoordinates(at.x, at.y);
    if (cell?.piece) cell.piece = null;
  }
}
