...
export class EventsExecutor {
  ...
  async executeEventsQueue(
    ...
    enableSounds = true,
    showLaser = true,
    timeout: number = this.eventsExecutionTimeout
  ): Promise<void> {
    for (const event of this.eventsQueue.filter(
      (e) => e.event_type != GameEvents.PIECE_DESTROYED_EVENT
    )) {
      if (event) {
        if (boardLogComponent && event.event_type != GameEvents.TELEPORT_EVENT)
          boardLogComponent.animatedHistorySelectionStep();
        await this.getAnimationToExecute(
          canvas,
          board,
          event,
          this.eventsQueue.indexOf(event),
          document.hidden ? false : showAnimations,
          enableSounds,
          showLaser
        );
  ...
  async executeLaserAnimations(
    canvas: Canvas,
    board: Board,
    laserPath: LaserShotEventEntity[],
    eventId: number,
    showAnimations: boolean,
    enableSounds: boolean,
    showLaser: boolean,
    laserShowDuration = 1000
  ): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      void (async () => {
        for (const path of allPaths)
          await Promise.all([
            this.animations.laserAnimation(
              canvas,
              board,
              path.map((p) => [p.from, p.to]),
              canvas.isReversed,
              showAnimations,
              enableSounds
            ),
            ...allDestroyedPieceEventsAfterLastLaserShot
              .filter(
                (e) =>
                  e.event_type == GameEvents.PIECE_DESTROYED_EVENT &&
                  e.laser_destroy_time == path[0].time
              )
              .map((e) =>
                this.animations.pieceDestroyedAnimation(
                  canvas,
                  board,
                  (<PieceDestroyedEvent>e).destroyed_on,
                  canvas.isReversed,
                  showAnimations,
                  enableSounds
                )
              ), // clears field as well so laser is cut.
          ]);
        ...
        allDestroyedPieceEventsAfterLastLaserShot.forEach(
          (pde) =>
            void this.animations.pieceDestroyedAnimation(
              canvas,
              board,
              (<PieceDestroyedEvent>pde).destroyed_on,
              canvas.isReversed,
              false,
              false
            )
  ...
  getAnimationToExecute(
    ...
    switch (gameEvent.event_type) {
      case GameEvents.PIECE_ROTATED_EVENT:
        return this.animations.rotatePiece(
          canvas,
          board,
      ...
      case GameEvents.PIECE_MOVED_EVENT:
        return this.animations.movePiece(
          canvas,
          board,
          gameEvent.moved_from,
          gameEvent.moved_to,
          canvas.isReversed,
          showAnimations,
          enableSounds
        );
      case GameEvents.TELEPORT_EVENT:
        return this.animations.movePiece(
          canvas,
          board,
          gameEvent.teleported_from,
          gameEvent.teleported_to,
          canvas.isReversed,
          showAnimations,
          enableSounds
        );
      case GameEvents.LASER_SHOT_EVENT:
        return this.executeLaserAnimations(
          canvas,
          board,
          gameEvent.laser_path,
          eventId,
          showAnimations,
          enableSounds,
          showLaser
  ...
  private startPath(
    nextTime: LaserShotEventEntity[] | undefined,
    allEvents: LaserShotEventEntity[],
    firstElem: LaserShotEventEntity,
    savePlace: PathInfo[]
  ) {
    if (!nextTime) return;
    ...
    for (const t of nextTime) {
      if (this.checkIfCoordinatesAreRightAngle(firstElem.coordinates, t.coordinates)) {
        ...
        const newPath = this.buildPath(
          allEvents.filter((ae) => ae.time == t.time + 1),
          allEvents,
          [firstElem, t],
          savePlace
        );
        if (newPath)
          this.startPath(
            allEvents.filter((ae) => ae.time == newPath.time + 1),
            allEvents,
            newPath,
            savePlace
          );
  ...
  private buildPath(
    ...
    if (!nextTime) return currentPath.slice(-1)[0];

    const newElem = this.getPathContinuation(currentPath, nextTime);
    if (!newElem) return currentPath.slice(-1)[0];

    savePlace.push({
      from: currentPath.slice(-1)[0].coordinates,
      to: newElem.coordinates,
      time: newElem.time,
    });
    return this.buildPath(
      allEvents.filter((ae) => ae.time == newElem.time + 1),
      allEvents,
      [...currentPath, newElem],
      savePlace
    );
  ...
  private checkIfCoordinatesAreRightAngle(coor1: Coordinates, coor2: Coordinates): boolean {
    return (
      (coor1.x - coor2.x == 0 && Math.abs(coor1.y - coor2.y) == 1) ||
      (coor1.y - coor2.y == 0 && Math.abs(coor1.x - coor2.x) == 1)
    );
  }

  private getPathContinuation(
    currentPath: LaserShotEventEntity[],
    allNextTimeEvents: LaserShotEventEntity[]
  ): LaserShotEventEntity | undefined {
    return allNextTimeEvents.find((e) => this.checkIfContinuationExists(currentPath, e));
  }

  private checkIfContinuationExists(
    currentPath: LaserShotEventEntity[],
    newElem: LaserShotEventEntity
  ) {
    const lastTwoElems = currentPath.slice(-2);
    const lastTwoElemsSubstraction = this.coordinatesSubstraction(
      lastTwoElems[0].coordinates,
      lastTwoElems[1].coordinates
    );
    const lastElemAndNewElemSubstraction = this.coordinatesSubstraction(
      lastTwoElems[1].coordinates,
      newElem.coordinates
    );
    return (
      lastElemAndNewElemSubstraction.x == lastTwoElemsSubstraction.x &&
      lastElemAndNewElemSubstraction.y == lastTwoElemsSubstraction.y
    );
  }
