...
export class Game {
  ...
  async initGame(
    ...
    animations: boolean,
    sounds: boolean,
    clocks: QueryList<ClockComponent>,
    theme: Theme
  ): Promise<void> {
  ...
  async loadDisplay(displaySize: number, receivedGameState: GameState): Promise<void> {
    if (this.gameCanvas && this.gameActions) {
      ...
      if (this.board.playerNum == PlayerType.PLAYER_TWO) this.flipBoard();
      ...
      this.playerNames = this.gameCanvas.isReversed
        ? [this.board.playerTwo, this.board.playerOne]
        : [this.board.playerOne, this.board.playerTwo];
    ...
    if (this.gameCanvas) {
      ...
      if (animationsToShow > 0)
        await this.executePendingActions(
          newGameState.game_events,
          animationsToShow,
          this.showAnimations,
          this.enableSounds
        );
      else this.loadConcreteGameState(newGameState);
  ...
  async gameRankingChanges(gameState: GameState): Promise<void> {
    if (
      this.gameId &&
      gameState.game_phase != GamePhase.NOT_STARTED &&
      gameState.game_phase != GamePhase.STARTED
    ) {
      const info = await this.gameService.getGameInfo(this.gameId);
      if (
        info.player_one_rating &&
        info.player_one_new_rating &&
        info.player_two_rating &&
        info.player_two_new_rating
      ) {
  ...
  async refreshGameState(newGameState: GameState): Promise<void> {
    if (this.gameId && this.gameCanvas) {
      if (this.analyzeMode != analyzeModes.ANALYZING) {
        if (!this.isInitiated) void this.loadDisplay(this.displaySize, newGameState);
        else if (this.analyzeMode == analyzeModes.EXITING_ANALYZE_MODE) {
          this.loadConcreteGameState(newGameState);
          this.analyzeMode = analyzeModes.NOT_ANALYZING;
        } else await this.loadNewGameState(newGameState);

        if (newGameState.game_phase != GamePhase.STARTED) this.gameCanvas.interactable = false;
      }
    } else console.error('Board not properly initialized');
    ...
    if (this.gamePhase == GamePhase.STARTED) {
      ...
      this.activeTurn = this.gameCanvas
        ? this.gameCanvas.isReversed
          ? [!p1Turn, p1Turn]
          : [p1Turn, !p1Turn]
        : [false, false];
    } else this.activeTurn = [false, false];

    this.playerTimes = this.gameCanvas?.isReversed
      ? [newGameState.player_two_time_left, newGameState.player_one_time_left]
      : [newGameState.player_one_time_left, newGameState.player_two_time_left];
  ...
  async showGameEvent(
    ...
    if (this.gameCanvas) {
      ...
      if (
        gameEvents.slice(-1)[0].event_type == GameEvents.LASER_SHOT_EVENT ||
        gameEvents.slice(-1)[0].event_type == GameEvents.PIECE_DESTROYED_EVENT
      )
        for (let i = gameEvents.length - 1; i > 0; i--)
          if (gameEvents[i].event_type == GameEvents.LASER_SHOT_EVENT) {
            ...
            void this.eventsExecutor.executeLaserAnimations(
              this.gameCanvas,
              this.board,
              ((<unknown>gameEvents[i]) as LaserShotEvent).laser_path,
              0,
              false,
              false,
              true,
  ...
  private async executePendingActions(
    events: GameEvent[],
    animationsToShow: number,
    showAnimations: boolean,
    enableSounds: boolean,
    boardLogComponent: BoardLogComponent | undefined = undefined,
    showLaser = true
  ) {
    if (this.gameCanvas) {
      this.gameCanvas.interactable = false;
      this.eventsExecutor.addEventsToExecute(events.slice(-animationsToShow));
      await this.eventsExecutor.executeEventsQueue(
        this.gameCanvas,
        this.board,
        boardLogComponent,
        showAnimations,
  ...
  giveUp(): void {
    if (this.gameId) this.gameService.giveUp(this.gameId);
  }

  offerDraw(): void {
    if (this.gameId) this.gameService.offerDraw(this.gameId);
  }
  ...
  passRotation(degree: number): void {
    if (this.gameActions) void this.gameActions.rotationPressed(this.board, degree);
  }

  passLaserShoot(): void {
    if (this.gameActions) this.gameActions.laserButtonPressed(this.board);
  }

  passAccept(): void {
    if (this.gameActions) this.gameActions.acceptRotationButtonPressed(this.board);
  }
