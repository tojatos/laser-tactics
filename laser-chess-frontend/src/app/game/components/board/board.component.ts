...
export class BoardComponent implements AfterViewInit, OnDestroy {
  ...
  buttonPressEvent(event: string): void {
    switch (event) {
      case 'left':
        this.game.passRotation(-90);
        break;
      case 'right':
        this.game.passRotation(90);
        break;
      case 'laser':
        this.game.passLaserShoot();
        break;
      case 'accept':
        this.game.passAccept();
        break;
    }
  }
  ...
  buildEvent(gameInfo: [GameEvent[], boolean]): void {
    ...
    void this.game.showGameEvent(
      gameInfo[0],
      this.game.lastPassedGameStateSize > 0,
      gameInfo[1],
      this.boardLogComponent
    );
  ...
  get containerHeight(): number {
    if (!this.isHandset) return this.currentSize * ROWS;
    else return this.currentSize * ROWS * this.handsetScale;
  }

  get containerWidth(): number {
    if (!this.isHandset) return this.currentSize * COLS;
    else return this.currentSize * COLS * this.handsetScale;
  }

  get displayedContainerWidth(): number {
    if (this.isHandset) return this.containerWidth;
    else return Math.max(this.containerWidth * this.placeForGameLogSize, this.minWidth);
  }
  ...
  get isSpectator(): boolean {
    return (
      this.game.authService.getUsername() != this.game.playerNames[0] &&
      this.game.authService.getUsername() != this.game.playerNames[1]
    );
  }
