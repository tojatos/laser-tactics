...
import {
  authWebsocketEndpoint,
  gameHistoryFullEndpoint,
  gameStateEndpoint,
  giveUpEndpoint,
  initialGameStateFullEndpoint,
  movePieceEndpoint,
  observeWebsocketEndpoint,
  offerDrawEndpoint,
  rotatePieceEndpoint,
  shootLaserEndpoint,
  spectatorsEndpoint,
  timeoutEndpoint,
} from 'src/app/api-definitions';
...
export class GameWebsocketService {
  ...
  connect(gameId: string) {
      ...
      .subscribe(
        (msg) => {
          if ((<websocketResponse>msg).status_code && (<websocketResponse>msg).status_code != 200) {
            ...
            if (this.lastMessage) this.eventEmitter.invokeRollback(this.lastMessage);
    ...
    if (token) this.sendRequest(authWebsocketEndpoint, { token: token });
  ...
  timeout(gameId: string, player: number): void {
    const request = { game_id: gameId, player_nr: player };
  ...
  showDrawOffer(gameId: string): void {
    if (this.lastMessage?.game_phase == GamePhase.STARTED)
      void Swal.fire({
        ...
        if (res.isConfirmed) this.offerDraw(gameId);
      });
  }
