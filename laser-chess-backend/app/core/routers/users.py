...
@router.get("/{username}/isActive", response_model=schemas.Stats)
def get_active(username: str, db: Session = Depends(get_db)):
    db_user = crud.get_user(db, username=username)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return crud.get_active(db=db, user=db_user)
...
@router.put("/me/logout")
def set_inactive(current_user: schemas.User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    return crud.set_active(db=db, user=current_user, active=False)