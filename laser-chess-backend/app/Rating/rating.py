...
def update_rating(player_data: PlayerRatingUpdate, match_history: PlayerMatchHistory):
    ...
    updated_player_rating = PlayerRatingUpdate(rating=round(player.rating, 2), rating_deviation=round(player.rd, 5),
                                               volatility=round(player.vol, 5))
